...
3. **Vérification** : déclenchement manuel avec `kubectl create job --from=cronjob/...`, confirmant que le nettoyage fonctionne réellement.
...
- `kubectl create job --from=cronjob/<nom>` permet de déclencher immédiatement une exécution d'un CronJob, sans attendre sa prochaine planification — pratique aussi bien pour tester que pour rattraper une exécution manquée.
