...
- **CronJob planifié toutes les minutes (`*/1 * * * *`)** : fréquence
  volontairement élevée pour que l'élève ait de bonnes chances
  d'observer au moins un échec "naturel" (Job créé automatiquement par
  la planification) en plus du déclenchement manuel demandé à l'étape 2.
  `step1.md` reste toutefois utilisable même si aucun Job planifié ne
  s'est encore déclenché (le diagnostic via `kubectl auth can-i` ne
  dépend pas du timing).
...
- Le temps de pull de l'image `bitnami/kubectl:latest` n'a pas pu être
  ...
  ou manuel) est lent à cause du pull d'image, le timeout de 90s dans
  `step2-verify.sh` pourrait s'avérer un peu juste — à ajuster après un
  premier test réel.
