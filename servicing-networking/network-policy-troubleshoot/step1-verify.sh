#!/bin/bash
CHEWIE_IP=$(kubectl get pod chewie -n b -o jsonpath='{.status.podIP}' 2>/dev/null)
if [ -z "$CHEWIE_IP" ]; then
  echo "Impossible de récupérer l'IP du pod chewie."
  exit 1
fi

HTTP_CODE=$(kubectl exec han -n a -- curl -s -o /dev/null -w "%{http_code}" --max-time 5 "http://${CHEWIE_IP}" 2>/dev/null)
